...
class c_color {
public:
	c_color() : r(255), g(255), b(255), a(255) { } /* default white */
	...
	__forceinline c_color transform_hex(std::string hex_color) {
		if (hex_color[0] == '#') {
			hex_color.erase(0, 1);
		}
	...
	static float hue(const c_color color) {
	...
	static float saturation(const c_color color) {
		float R = base(color.r);
		float G = base(color.g);
		float B = base(color.b);
		float mx = max(R, max(G, B));
	...
	static float brightness(const c_color color) {
		float R = base(color.r);
		float G = base(color.g);
		float B = base(color.b);
	...
	__forceinline float* transform() {
		static float value[4];

		value[0] = this->r / 255.f;
		value[1] = this->g / 255.f;
		value[2] = this->b / 255.f;

		return value;
	}

	static c_color from_hsb(float hue /* 0.f - 1.f*/,
		...
		if (h < 1.f)
			return c_color(brightness * 255, t * 255, p * 255, alpha);

		else
			if (h < 2.f)
				return c_color(q * 255, brightness * 255, p * 255, alpha);

			else
				if (h < 3.f)
					return c_color(p * 255, brightness * 255, t * 255, alpha);

				else
					if (h < 4)
						return c_color(p * 255, q * 255, brightness * 255, alpha);

					else
						if (h < 5)
							return c_color(t * 255, p * 255, brightness * 255, alpha);

						else
							return c_color(brightness * 255, p * 255, q * 255, alpha);
	}

	c_color blend(const c_color& other, float fraction) const {
		int r_d = r - other.r;
		int g_d = g - other.g;
		int b_d = b - other.b;
		int a_d = a - other.a;
	...
	c_color darker(int amount) {
		...
		return c_color(red, green, blue, a);
	}
	...
	static c_color hsv_to_rgb(double h, double s, double v) {
		double      hh, p, q, t, ff;
		long        i;
		rgb         out;
		...
		if (s <= 0.0) {
			...
			out.g = v;
			out.b = v;
			return c_color(out.r * 255.f, out.g * 255.f, out.b * 255.f);
		}
		hh = h;
		if (hh >= 360.0) hh = 0.0;
		hh /= 60.0;
		...
		return c_color(int(out.r * 255.f), int(out.g * 255.f), int(out.b * 255.f));
	}
