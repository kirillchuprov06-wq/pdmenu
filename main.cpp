...
#include "main.hpp"
#include "framework/render/hxd/image_backround.hpp"
...
inline IDirect3DTexture9* bgs = nullptr;
...
int main(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	g_render_engine->intialize_renderer( g_pd3dDevice, g_pWindow );

	if ( bgs == nullptr )
		D3DXCreateTextureFromFileInMemoryEx( g_pd3dDevice, export_hxd, sizeof( export_hxd ), 1920, 1080, D3DX_DEFAULT, D3DUSAGE_DYNAMIC, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &bgs );
	...
	while (msg.message != WM_QUIT) {
		...
		ImGui::NewFrame( );
		{
			...
			ImGui::GetBackgroundDrawList( )->AddImage( bgs, ImVec2( 0, 0 ), ImVec2(1920, 1080) ); // 2560, 1440

			g_font.verdana.string( 10, 10, "vagon menu framework", c_color( 255, 255, 255 ) );
			g_menu->run( );
		}
		ImGui::EndFrame( ); 
...
void SetupRenderStates(std::function< void() > func) {
	...
	c_vector_2d DisplaySize = c_vector_2d((float)(rect.right - rect.left), (float)(rect.bottom - rect.top));
